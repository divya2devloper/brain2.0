...
class SupabaseClient:
    ...
    def insert(self, table: str, payload: dict[str, Any], service: bool = True) -> Any:
        with httpx.Client(timeout=15.0) as client:
            r = client.post(f"{self.base_url}/rest/v1/{table}", headers=self._headers(service=service), json=payload)
            r.raise_for_status()
            return r.json()
    ...
    def update(self, table: str, pk_field: str, pk_value: str, tenant_field: str, tenant_id: str, payload: dict[str, Any]) -> Any:
        ...
        with httpx.Client(timeout=15.0) as client:
            r = client.patch(f"{self.base_url}/rest/v1/{table}?{query}", headers=self._headers(service=True), json=payload)
            r.raise_for_status()
            return r.json()
