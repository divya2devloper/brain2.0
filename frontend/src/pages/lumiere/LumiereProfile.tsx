...
/* Trial modal shown on first visit */
function TrialModal({ onClose }: { onClose: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(10,10,30,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 420, padding: '32px 36px', boxShadow: '0 24px 64px rgba(0,0,0,0.18)', textAlign: 'center' }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>🎉</div>
        <h2 style={{ margin: '0 0 8px', fontWeight: 800, fontSize: 22, color: '#1a1a2e' }}>Welcome to Lumière!</h2>
        <p style={{ fontSize: 14, color: '#666', margin: '0 0 16px', lineHeight: 1.6 }}>
          You have been granted a <strong>72-hour full-access trial</strong>. Explore all features including revenue analytics, advanced job management, and unlimited freelancer assignments.
        </p>
        <div style={{
          background: 'linear-gradient(135deg, #ff8c00, #ffc107)', borderRadius: 12,
          padding: '16px 20px', marginBottom: 20, color: '#fff',
        }}>
          <div style={{ fontWeight: 800, fontSize: 28 }}>72h 0m</div>
          <div style={{ fontSize: 13, opacity: 0.9 }}>Full access remaining</div>
        </div>
        <button onClick={onClose} style={{
          width: '100%', padding: '12px 0', borderRadius: 10, border: 'none',
          background: 'linear-gradient(90deg, #1dbde6, #4a9eff)',
...
const TRIAL_SHOWN_KEY = 'lumiere_trial_shown'

export function LumiereProfile() {
  const { viewMode, setViewMode, authLevel, setAuthLevel } = useLumiere()
  const [remaining, setRemaining] = useState(() => getTrialExpiry() - Date.now())
  const [showTrialModal, setShowTrialModal] = useState(() => !localStorage.getItem(TRIAL_SHOWN_KEY))

  useEffect(() => {
    if (showTrialModal) localStorage.setItem(TRIAL_SHOWN_KEY, '1')
  }, [showTrialModal])

  useEffect(() => {
    const id = setInterval(() => setRemaining(getTrialExpiry() - Date.now()), 60000)
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18, paddingBottom: 20, borderBottom: '1px solid #f0f4f8' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                ...
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#888', padding: 2 }}><EditIcon /></button>
              </div>
              ...
              <div style={{ display: 'flex', gap: 18, fontSize: 13, color: '#666' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><MailIcon /> studio@lumiere.in</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><PhoneIcon /> +91 98765 43210</span>
              </div>
            </div>
          </div>
            ...
            <>
              <div style={{ paddingTop: 20, paddingBottom: 20, borderBottom: '1px solid #f0f4f8' }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {skills.map(s => (
                    <span key={s} style={{ background: '#f5e6ff', color: '#9333ea', border: '1px solid #e8c8ff', borderRadius: 20, padding: '5px 14px', fontSize: 13, fontWeight: 500 }}>{s}</span>
                  ))}
                </div>
              </div>
              <div style={{ paddingTop: 20 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {equipment.map(e => (
                    <div key={e} style={{ background: '#fff8f0', border: '1px solid #ffe8cc', borderRadius: 8, padding: '10px 14px', fontSize: 14, color: '#333' }}>{e}</div>
                  ))}
                </div>
              </div>
            </>
            ...
            <div style={{ paddingTop: 20 }}>
              ...
              {branches.map((b, i) => (
                <div key={b.city} style={{
                  ...
                  <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#aaa', padding: 4 }}><EditIcon /></button>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e8ecf0', padding: '20px 22px' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {(['studio', 'freelancer'] as const).map(m => (
                  <button key={m} onClick={() => setViewMode(m)} style={{
                    ...
                    background: viewMode === m ? 'linear-gradient(90deg, #1dbde6, #4a9eff)' : '#fff',
                    color: viewMode === m ? '#fff' : '#555',
                  }}>{m === 'studio' ? 'Studio Owner' : 'Freelancer'}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {(['manager', 'staff'] as const).map(a => (
                  <button key={a} onClick={() => setAuthLevel(a)} style={{
                    flex: 1, padding: '8px 0', borderRadius: 8, border: '1px solid #e8ecf0',
                    cursor: 'pointer', fontSize: 13, fontWeight: authLevel === a ? 700 : 400,
                    background: authLevel === a ? 'linear-gradient(90deg, #e040fb, #c0392b)' : '#fff',
                    color: authLevel === a ? '#fff' : '#555',
                  }}>{a === 'manager' ? 'Manager' : 'Staff'}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{ background: '#fffbec', borderRadius: 14, border: '1px solid #ffe8a0', padding: '20px 22px' }}>
            ...
            <div style={{ fontSize: 13, color: remaining > 0 ? '#888' : '#e53e3e' }}>
              {remaining > 0 ? 'Full access remaining' : 'Trial expired'}
            </div>
          </div>
        </div>
      </div>
